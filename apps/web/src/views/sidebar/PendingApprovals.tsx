...
export function PendingApprovals({ approvals, isLoading, isPending, onApprove, onReject }: Props) {
  if (isLoading) {
    return (
  ...
  if (approvals.length === 0) {
    return <div className="p-4 text-center text-sm text-gray-400">No pending approvals</div>;
  }
        ...
        <div key={approval.id} className="p-3 border border-amber-200 bg-amber-50 rounded-lg">
          ...
          <div className="flex gap-1">
            <button
              ...
              disabled={isPending}
              className="flex-1 text-xs border border-red-200 text-red-600 rounded py-1 hover:bg-red-50 disabled:opacity-50"
            >
              Reject
            </button>
            <button
